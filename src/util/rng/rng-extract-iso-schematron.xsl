...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:param name="default-namespace" as="xs:string"
		select="'http://www.daisy.org/ns/z3998/authoring/'" />
	...
	<xsl:template match="/">
		<schema queryBinding="xslt2">
			...
			<xsl:element name="xsl:key">
				...
				<xsl:attribute name="match">default:document</xsl:attribute>
				<xsl:attribute name="use">document(//default:meta[@rel='z3998:rdfa-context']/@resource)//*[@property='rdfa:prefix']/text()</xsl:attribute>
			</xsl:element>
			...
			<pattern id="id-unique">
				...
				<rule context="*[@xml:id]">
					<assert test="count($id-set[@xml:id = current()/@xml:id]) = 1"
						>Duplicate ID '<sch:value-of select="current()/@xml:id"/>'</assert>
				</rule>
			</pattern>
