			...
			<named-elements>
				<xsl:for-each select="/grammar/define/element[@name]">
					<xsl:copy copy-namespaces="no">
						...
						'00'),
						'.html')" />
						<xsl:if test="descendant::dcterms:title/@z:short">
							<xsl:attribute name="zsd:context">
								<xsl:choose>
									...
									<xsl:otherwise>
										<xsl:value-of select="descendant::dcterms:title/@z:short" />
									</xsl:otherwise>
								</xsl:choose>
							</xsl:attribute>
						</xsl:if>
		...
		<rng-attribute-list xmlns="http://relaxng.org/ns/structure/1.0">
			...
			<xsl:for-each select="/grammar/define/attribute[@name]">
				<xsl:copy copy-namespaces="no">
					...
					<xsl:if test="preceding-sibling::rdf:RDF or following-sibling::rdf:RDF">
						<xsl:message>RDF for <xsl:value-of select="@name" /></xsl:message>
				...
				<body class="attributeInformation">
					...
					<xsl:for-each
						...
						<div class="attributeInformation"
							...
							<!-- Description -->
							<xsl:if test="descendant::dcterms:description">
								<h3>Description</h3>
								<xsl:apply-templates
									select="descendant::dcterms:description,descendant::z:info" />
							</xsl:if>
								...
								<xsl:for-each
									...
									<div class="attributeInformation"
										...
										<h2>
											...
											<xsl:text> RDFa attribute</xsl:text>
										</h2>
			...
			<p>
				<xsl:choose>
					...
					<xsl:when test="count($elements) eq 1">
						<xsl:text>This attribute occurs on on the </xsl:text>
					...
					<xsl:otherwise>
						<xsl:text>This attribute occurs on on the following elements: </xsl:text>
						<xsl:call-template name="presentElementnamesWithLinks">
							<xsl:with-param name="elems" as="element()+" select="$elements" />
						</xsl:call-template>
					</xsl:otherwise>
	...
	<xsl:template name="generateXhtmlFileForElement">
		...
		<xsl:variable name="element.core.info" as="element()">
			<div class="element-coreinfo" xmlns="http://www.w3.org/1999/xhtml">
				...
				<xsl:if test="descendant::dcterms:description or child::a:documentation">
					<div class="main-description">
						<!--<h2>Description</h2>-->
						<xsl:choose>
							<xsl:when test="descendant::dcterms:description">
								<xsl:apply-templates
									select="descendant::dcterms:description, descendant::z:info" />
							</xsl:when>
...
-->
	...
	<xsl:template match="dcterms:description | z:info | a:documentation">
