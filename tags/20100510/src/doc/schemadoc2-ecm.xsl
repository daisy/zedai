...
<xsl:stylesheet exclude-result-prefixes="xs rng xd dcterms z rdf zrng zsd a x sch" version="2.0"
	...
	<xsl:template match="grammar/optional[not(descendant::element)][count(child::*) eq 1 and group][every $e in group/child::* satisfies local-name($e) eq 'optional']" mode="ecm-clean">
	...
	<xsl:template match="group[count(child::*) gt 1 and count(descendant::element) gt 1]/*" mode="ecm-order">
		<xsl:copy copy-namespaces="no">
			<xsl:copy-of select="@*" />
			<xsl:attribute name="order-text">
				<xsl:choose>
