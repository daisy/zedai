...
<html xmlns="http://www.w3.org/1999/xhtml" xmlns:xi="http://www.w3.org/2001/XInclude">  
  ...
  <div class="intro">
    <p>The <code>Print Forms</code> feature provides a mechanism with which publishers can define 
      form elements to represent a subset of the various form objects present in print publications.  
      These form objects are best identified via the two primary use-cases for this feature:</p>
    ...
    <p>In addition to defining form elements, this feature provides the functionality of defining
      methods of submission for the collection of entered information.  In the context of 
      <code>Print Forms</code>, submission is broken into two parts: method and instance.  The 
      method defines the &quot;type&quot; of submission for the document, with options ranging from 
      saving the form input as a file to transmitting over the network to an author-specified server.
      Comprised by the <code>&lt;instance&gt;</code> element, the instance information defines the 
      structure of the sent information.  This structure is an XML format of the author's choice.  
      For example, a section of a document with form elements for entering a name and date:</p>
    <pre>
      &lt;xforms:input&gt;
    ...
    <p>where the &quot;Today's date&quot; information is placed as an attribute for the <code>&lt;user&gt;</code> 
      element, and the &quot;First Name&quot; and &quot;Last Name&quot; information is placed in the 
      <code>&lt;firstName&gt;</code> and <code>&lt;lastName&gt;</code> elements, repsectively.</p>
    <p>Including submission-related items is entirely an author-level decision, as many factors of the 
      authoring environment are involved.  In general, to determine if submission-related items should be 
      included in a document, both of the following should be true:</p>
    <ul>
      <li>The purpose of the document is to retreive information (such as an application form or a consumable 
        textbook).</li>
      <li>The author has a set location for retreiving the information (such as a web server) or intends for 
        the information to be saved locally to the user's computer.</li>
    </ul>
    <p>As an example, a primary grade textbook which contains form elements only for the purpose of personal 
      inquiry on the reader's behalf would not fit the first criterion above, thus submission-related
      elements should not be included.  However, an employment application form is expected to retrieve 
      information and, in this case, an author would have a set location for retrieving the information
      (such as the same web server used for acquiring the application's web form analogue).</p>
  ...
  <div class="examples">
    <p>Each of the following examples depicts a real-world application of the <code>Print Forms</code>
      feature along with a short reasoning of their formulation.</p>
    ...
    <ul>
      ...
      <li>XML comments (<code>&lt;!-- --&gt;</code>) are used in the code examples to identify points
        of interest and further explain items referenced in the Discussion.</li>
    </ul>
    <h3>Example 1</h3>
    <img src="resources/example01.jpg" alt="Print textbook example 1 for use with the Print Forms feature" />
    <p>This example is of a page from a primary grade consumable.  The student is expected to answer 
      ...
      is a key part of this example, the optional <code>&lt;model&gt;</code> element and its children 
      are included.</p>
    <p>The first two questions are encoded as an <code>&lt;input&gt;</code> element, which represents 
      a free text input, analagous to a HTML text field.  The last question is encoded as a <code>&lt;select1&gt;</code> 
      element, which represents a &quot;select one of many choices&quot; input, analagous to a HTML 
      pull-down list field.</p>
    <p>Each of the different types of inputs contain an optional <code>ref</code> attribute, which links 
      the user-submitted information to a point in the optional <code>&lt;instance&gt;</code> 
      section.</p>
    <pre>
&lt;?xml version="1.0" encoding="UTF-8"?&gt;
&lt;html xmlns="http://www.daisy.org/ns/z3986/authoring/" 
  xmlns:xforms="http://www.w3.org/2002/xforms/" 
  xmlns:dc="http://dublincore.org/quuqz#"
  xmlns:my="http://www.daisy.org/ns/z3986/authoring/xforms/instance/"
  xml:lang="en-US"&gt;
  &lt;head&gt;
    &lt;meta property="dc:title"&gt;Print Forms Feature Example 1&lt;/meta&gt;
    &lt;xforms:model&gt;
      &lt;xforms:instance&gt;
...
&lt;/html&gt;
    ...
    <h3>Example 2</h3>
    <img src="resources/example02.jpg" alt="Print textbook example 2 for use with the Print Forms feature" />
    <p>This example is of a section of a page from a primary grade consumable.  As with the previous 
      example, the student is expected to answer the question on the print page and submit it to his/her 
      instructor.  Since submission is a key part of this example, the optional <code>&lt;model&gt;</code> 
      element and its children are included.</p>
    <p>Of particular note in this example is that the question is actually comprised of two sub-questions
      (as evidenced by the phrase &quot;show your work&quot;). For the purposes of the example, it is assumed 
      that both sub-questions are required to be answered, with no room for partial credit.  Given this 
      assumption, the <code>&lt;bind&gt;</code> element is used, which provides additional rules on 
      submission (in this case, forcing both questions to be answered).</p>
...
&lt;html xmlns="http://www.daisy.org/ns/z3986/authoring/" 
  ...
  xml:lang="en-US"&gt;
  &lt;head&gt;
    &lt;meta property="dc:title"&gt;Print Forms Feature Example 2&lt;/meta&gt;
    &lt;xforms:model&gt;
      &lt;xforms:instance&gt;
        &lt;my:worksheet&gt;
          &lt;my:questions&gt;
...
&lt;/html&gt;
    ...
    <h3>Example 3</h3>
    <img src="resources/example03.jpg" alt="Print textbook example 3 for use with the Print Forms feature" />
    <p>This example is of a section of a page from a primary grade textbook.  Unlike the previous examples, 
      the questions posed in the text do not require submission and instead are presented for mental reference.
      Since answer submission is not necessary in this example, the optional <code>&lt;model&gt;</code> element 
      is not included, leaving only the actual form controls.  In addition to not including 
      <code>&lt;model&gt;</code>, the <code>ref</code> attribute, which links form elements to the submission
      structure, is also not included on the individual <code>&lt;input&gt;</code> elements.</p>
    <p>The first section of this example is similar to the previous examples, with two <code>&lt;input&gt;</code>
      elements.  The second section, however, involves some re-formatting of the content.  In print, the paragraph
      is composed of a series of labeled markers which appear inline.  Each of these labeled markers correspond 
      to input lines present following the paragraph.  In the <code>Print Forms</code> feature, this separation 
      isn't necessary (and may also be confusing in certain circumstances).  As such, the form inputs are 
...
&lt;html xmlns="http://www.daisy.org/ns/z3986/authoring/" 
  ...
  xml:lang="en-US"&gt;
  &lt;head&gt;
    &lt;meta property="dc:title"&gt;Print Forms Feature Example 3&lt;/meta&gt;
