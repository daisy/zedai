 ...
 * (the cmp document in the spec) and xhtml (resource directories)
 * 
 * The key is the <?absdef src="" ?> PI embedded in the input. 
 * 
 * Note that the spec and cmp must be built before schemadocgen and rdgen 
 * (need the temp docbook spec path to resolve link labels)
 * 
 */

public class AbstractDefExpander {	
	private static final String WWW_SCHEMA_BASE_DIR = "http://www.daisy.org/z3986/2010/schema/";
...
//		}
	...
	private void expandAbsDef(RNGDocument rng, ProcessingInstruction absdef, RNGDocuments context) throws Exception {
		...
		//if module is required in all profiles, say so. Depends on <dcterms:isRequiredBy rdf:resource="http://www.daisy.org/z3986/2010/auth/"/>
	...
	private void buildOverviewModuleRequired(RNGDocument rng, Element section)
		...
		if(req) {
			...
			para.appendChild("This module must be included in all compliant Z39.86-2010 Profiles.");
			section.appendChild(para);
		}
	}
	...
	private Element buildElementOrAttributeDetailsSection(RNGElement element, RNGDocument owner, DefineType type, String tableID) throws Exception {
		...
		//details: localname
		detailsTable.addDataRow(new String[]{"LocalName", "{"+element.getAttributeValue(Constants.NAME)+"}"}, true, null);
		
		//details: namespace URI		
		detailsTable.addDataRow(new String[]{"Namespace",getNsString(element)}, true, null);
		
		//details: usage context
		detailsTable.addDataRow(new String[]{"Initial Usage Context",rdf.getContextDescription().getValue()}, true, null);
		
		//details: usage context fixed
		String contextFixed = rdf.getContextFixed();
		if(contextFixed!=null) {
			detailsTable.addDataRow(new String[]{"Usage Context Alterability",buildFixedString(contextFixed,rdf.getContextDescription(),element)}, true, null, null, ALTERABILITY);
		}
		
		//details: attribute model		
		if(rdf.getAttlistDescription()!= null) {
			detailsTable.addDataRow(new String[]{"Initial Attribute Model",rdf.getAttlistDescription().getValue()}, true, null);
			//details: attribute model fixed
			String attlistFixed = rdf.getAttlistFixed();
			if(attlistFixed!=null) {
				detailsTable.addDataRow(new String[]{"Attribute Model Alterability",buildFixedString(attlistFixed,rdf.getAttlistDescription(),element)}, true, null, null, ALTERABILITY);
			}
		}

		//details: content model				
		detailsTable.addDataRow(new String[]{"Initial Content Model",getContentDescription(rdf)}, true, null);
		
		//details: content model fixed
		String contentFixed = rdf.getContentFixed();
		if(contentFixed!=null) {
			detailsTable.addDataRow(new String[]{"Content Model Alterability",buildFixedString(contentFixed,rdf.getContentDescription(),element)}, true, null, null, ALTERABILITY);
		}
	 ...
	 */
	private String buildFixedString(String value, Element carrier, Element declared) {
		StringBuilder sb = new StringBuilder();
		
		// the value is either true, in which case the model is completely fixed,
		// or any other value, in which case it is partially fixed
		
		if(value.equals("true")) {			
			if(carrier.getLocalName().equals(Constants.ATTLIST)) {
				sb.append("This attribute model");
			}else if(carrier.getLocalName().equals(Constants.CONTEXT)) {
				sb.append("This usage context");
			}else if(carrier.getLocalName().equals(Constants.CONTENT)) {
				sb.append("This content model");
			}else {
				throw new IllegalStateException(carrier.toXML());
			}		
			sb.append(" is fixed, and must not be altered when activating this module.");
		}else {
			sb.append(value);
		}
		return sb.toString();
