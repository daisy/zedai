...
public class RDFDescriptionElement extends AnyElement {
	...
	/**
	 * Get the value of @fixed on the z:context element, or null of no such attribute exists. 
	 * Value is 'true' (total alterability restriction) or any other string (partial alterability restriction).
	 * Invocation is illegal on the module-level RDF.
	 */
	public String getContextFixed() {
		if(isModuleLevel()) throw new IllegalStateException();
		return getFixedAttributeValue(getContextDescription());
	}
	
	private String getFixedAttributeValue(Element parent) {
		if(parent!=null) {
			Attribute fixed = parent.getAttribute(Constants.FIXED);
			if(fixed!=null) {
	...
	/**
	 * Get the value of @fixed on z:content element, or null of no such attribute exists. 
	 * Value is 'true' (total alterability restriction) or any other string (partial alterability restriction).
	 * Invocation is illegal on the module-level RDF.
	 */
	public String getContentFixed() {
		if(isModuleLevel()) throw new IllegalStateException();
		return getFixedAttributeValue(getContentDescription());
	}
	...
	/**
	 * Get the value of @fixed on z:attlist element, or null of no such attribute exists. 
	 * Value is 'true' (total alterability restriction) or any other string (partial alterability restriction).
	 * Invocation is illegal on the module-level RDF.
	 */
	public String getAttlistFixed() {
		if(isModuleLevel()) throw new IllegalStateException();
		return getFixedAttributeValue(getAttlistDescription());
	}
	...
	public boolean isRequiredModule() {			
		if(!isModuleLevel()) throw new IllegalStateException();
		Boolean omittable;	
		Nodes nodes = this.query(".//dcterms:requiredBy[@rdf:resource='http://www.daisy.org/z3986/2010/auth/']", Constants.namespaces);
		if(nodes.size()==1) {
			omittable = Boolean.TRUE;
		}else if(nodes.size()==0) {
			omittable = Boolean.FALSE;
	...
	public List<RNGDocument> getExternalDependencies(RNGDocuments context) {	
		...
		Nodes deps = this.query(".//dcterms:requiredBy[not(@rdf:resource='http://www.daisy.org/z3986/2010/auth/')]", Constants.namespaces);				
