...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	* Better feature handling:
	...
	<xsl:variable name="tempFolder" as="xs:string"
		select="concat(replace(resolve-uri($docResourceURI),'^(.+/).+$','$1'),'tmp/')" />
	...
	<xsl:variable name="docResource.elements" as="element()*">
		<els xmlns="http://www.daisy.org/ns/z3986/2008/docResources/">
			<xsl:for-each select="$docResource.content//dr:element">
				...
				<element>
					<xsl:copy-of
						select="@name, @descriptionURI, @descriptionInclude, ../@nsURI, @context" />
					<xsl:if test="@context">
				...
				<attribute>
					...
					<xsl:if test="@context">
						<xsl:choose>
							...
							<xsl:otherwise>
								<xsl:attribute name="contextLocalName" select="@context" />
								<xsl:attribute name="contextNamespace" select="$defaultNamespace" />
							</xsl:otherwise>
						</xsl:choose>
						<!--						<xsl:attribute name="contextLocalName"
							select="substring-after(@context,':')" />
	...
	<!-- DEFINE OUTPUT -->
	<!-- psps20091008: Tatt bort xsl:output, må sjekke hvilke konsekvenser det får -->
	<xsl:output method="xhtml" indent="yes" doctype-public="-//W3C//DTD XHTML 1.0 Strict//EN"
		doctype-system="http://www.w3.org/TR/xhtml1/DTD/xhtml1-strict.dtd" />
	<xsl:output name="shortSamples" method="xml" omit-xml-declaration="yes" encoding="UTF-8" />

	<xsl:variable name="t" as="xs:anyURI" select="document-uri(.)" />
	<!-- TEMPLATE MATCHING ROOT -->
	<xsl:template match="/">
		<!--		<xsl:message><xsl:value-of select="document-uri(/)"/></xsl:message>
		<xsl:message><xsl:value-of select="$docResourceURI"/></xsl:message>-->
		...
		<xsl:call-template name="createShortSampleTempFiles" />
		<html xmlns="http://www.w3.org/1999/xhtml" lang="en">
			...
			<body style="margin-top:4em;margin-bottom:2em;">
				...
				<xsl:for-each-group
					select="/grammar/define[element/@name][not(hks:elementIsAFeatureNew(element))]"
					group-by="element/@ns">
				...
				<xsl:if test="hks:profileContainsFeatureElements(/grammar)">
					...
					<xsl:for-each-group
						select="/grammar/define[element/@name][hks:elementIsAFeatureNew(element)]"
