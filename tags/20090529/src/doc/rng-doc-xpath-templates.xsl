...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="*" mode="dr">
		<xsl:copy>
			...
			<xsl:if test="ancestor::dr:namespace/@feature eq 'yes'">
				<xsl:copy-of
					select="ancestor::dr:namespace/@feature, ancestor::dr:namespace/@feature-nicename"
				 />
			</xsl:if>
	...
	<xsl:template match="element">
		...
		<!--		<xsl:if test="false()  and matches(../@name,'^link$|^pagebreak$')">
			<xsl:result-document href="{concat('_ats_',../@name,'.xml')}">
		...
		<xsl:variable name="elementInformation" as="element()">
			<div>
				<div class="element-coreinfo">
					<!--					<h2>Attributes (alt)</h2>
					<xsl:apply-templates select="$attribs//group/*" mode="altacm"/>-->
					<!--					<h2>Element name</h2>
					<p>
						<span class="element">
...
select="resolve-uri(substring-before($relevant.doc/@docURI,'#'),base-uri($docResource.content))"/>-->
					...
					<!--					<h2>Complete Content Model</h2>
					<xsl:call-template name="presentCompleteContentModel"/>-->
...
<!--		<span class="rng-datatype">
	...
	<xsl:template match="param[@name eq 'pattern']" mode="acm">
		<div class="indent">
			...
			<xsl:text>"</xsl:text>
			<code><xsl:apply-templates mode="acm" /></code>
	...
	<xsl:template match="value" mode="acm">
		<code>
			<xsl:apply-templates mode="acm"></xsl:apply-templates>
		</code>
