...
<!--					<div>
	...
	<xsl:template name="listChildren">
		...
		<xsl:if test="($elements[anyName] and $elements[@name]) or ($elements[anyName] and $elements[nsName])">
			<xsl:text>, and </xsl:text>
		</xsl:if>
		<xsl:for-each select="$elements[anyName]">
			...
			<xsl:if test="anyName/except//name">
				...
				<xsl:for-each select="anyName/except//name">
					<code>
						<xsl:value-of select="hks:addNamespacePrefix(.,if (@ns eq '' or not(@ns)) then $defaultNamespace else @ns)" />
					</code>
					<xsl:choose>
						<xsl:when test="position() eq last()" />
						<xsl:when test="position() eq last()-1">
