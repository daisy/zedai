...
-->
	...
	<xsl:template match="element()" mode="shortsample">
		<!-- We need to be able to define wether we are processing the container-element for the sample (typically a pre-element, and certainly with an @id),
			so lets get the @id of the container as a parameter
		-->
		<xsl:param name="container-id" as="xs:string" select="'0000-illegal-id-value-0000'" />
		<xsl:choose>
			<xsl:when test="$container-id eq @id">
				<!-- We have matched the container of the sample, so we just continue processing  -->
