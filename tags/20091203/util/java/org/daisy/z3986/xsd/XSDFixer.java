...
public class XSDFixer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Document xsd = build(args[0], impl);

			xsd = normalizeRDFaAttributes(xsd);

			write(xsd, args[1], impl);
	...
	private static Document normalizeRDFaAttributes(Document xsd) {
		...
		try {
			...
			cnt.declareNamespace("xsd", XSD_NS);
			...
			String exp = 	
			    "//xsd:*[xsd:attribute[@name='about' and not(@use='required')] " +
					"and xsd:attribute[@name='content' and not(@use='required')] " +
					"and xsd:attribute[@name='datatype' and not(@use='required')] " +
					"and xsd:attribute[@name='typeof' and not(@use='required')] " +
					"and xsd:attribute[@name='property' and not(@use='required')] " +
					"and xsd:attribute[@name='rel' and not(@use='required')] "+ 
					"and xsd:attribute[@name='resource' and not(@use='required')] " +
					"and xsd:attribute[@name='rev' and not(@use='required')]]";
			...
			if(elems.getLength()>0) {
				...
				String exp2 = 	
				    "xsd:attribute[@name='about' or @name='content' or @name='datatype' or @name='typeof' " +
