...
<xsl:stylesheet exclude-result-prefixes="xs rng xd dcterms z rdf zrng zsd a x sch" version="2.0"
	...
	<!-- start mode: acm2-rdf -->
	<xsl:template match="rdf:RDF" mode="acm2-rdf">
		<div class="rdf">
			<xsl:apply-templates mode="acm2-rdf" />
		</div>
	</xsl:template>
	
	<xsl:template match="dcterms:title" mode="acm2-rdf">
		<strong>
			<xsl:apply-templates mode="acm2-rdf" />
			<xsl:text>:</xsl:text>
		</strong>
	</xsl:template>
	<xsl:template match="dcterms:description" mode="acm2-rdf">
		<xsl:apply-templates mode="acm2-rdf" />
	</xsl:template>
	...
	<xsl:template match="datatype/rdf:RDF" mode="acm2" />
	...
	<xsl:template match="data[@type and @datatypeLibrary]" mode="acm2">
		<div class="rng">
			...
			<xsl:apply-templates select="(following-sibling::rdf:RDF,preceding-sibling::rdf:RDF)[1]"
				mode="acm2-rdf" />
	...
	<xsl:template match="grammar/rdf:RDF" mode="acm2">REMOVED</xsl:template>
