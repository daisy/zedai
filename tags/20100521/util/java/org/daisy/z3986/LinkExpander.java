...
public class LinkExpander {
	 ...
	 * @throws Exception
	 */
	public LinkExpander(Document input, QNameProvider qnames, Document spec,
			Document cmp) throws Exception {
		this.input = input;
		this.qnames = qnames;
		this.spec = spec;
		this.cmp = cmp;
		...
		walk(input, new WalkerListener() {
			...
			private String getLabel(Element elem) {
				if(elem!=null) {			
					...
					Nodes nodes = elem.query(".//*[@z:short]", Constants.namespaces);
					if(nodes.size()>0) {
						return ((Element)nodes.get(0)).getAttributeValue(Constants.SHORT, Constants.Z_M12N_VOCAB_NS); 
					}
...
//					throw new IllegalStateException("failed getting a label from " + print.toXML());
				}
				return null;
			}
				...
				if (linkAttr != null && (linkAttr.getLocalName()==Constants.LINKEND || linkAttr.getValue().trim().startsWith("#"))) {
					...
					if (!inputIDMap.containsKey(id)) {
						...
						if (specIdMatch == cmpIdMatch) {
							if(specIdMatch) {
								System.err.println("ERROR (LinkExpander): spec and cmp both matches id " + id);
								specIdMatch = false; //choose the CMP match
							}else{
								System.err.println("ERROR (LinkExpander): neither spec nor cmp matches id " + id);
								return;
							}
						}
