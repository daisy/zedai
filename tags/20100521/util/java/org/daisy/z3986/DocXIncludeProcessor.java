...
public class DocXIncludeProcessor {
	...
	private Document process(File input, boolean isLongdescMode) throws Exception {
		...
		//find each root child element with @about (which is the indicator we use)
	...
	private void pushContents(Element elem) throws Exception {
		...
		if(contents.trim().length()==0) {
			System.err.println("Error: empty string returned on " + elem.toXML());			
		}
	...
	/**
	 * Get the contents of incoming snippet as a string.
	 * @param elem
	 * @return
	 * @throws Exception 
	 */
	private String getContents(Element elem) throws Exception {
		
		StringBuilder sb = new StringBuilder();
				
		if(elem.getChildElements().size() == 1 
				&& elem.getChildElements().get(0).getLocalName() == "document"){
			//if this snippet has only one child named document, reinsert the default ns decl
			Element document = elem.getChildElements().get(0);
			Element copy = (Element) document.copy();
			return tweakLineLength("\n" + copy.toXML());
		}else {	
			//default behavior
		...
		return tweakLineLength(sb.toString());
	}

	private String tweakLineLength(String xml) {	 
		return xml.replace("\" xmlns:", "\"\n          xmlns:");
	}
