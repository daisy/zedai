...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:d="http://www.daisy.org/z3998#" xmlns:rng="http://relaxng.org/ns/structure/1.0"
	xmlns:sel="http://www.daisy.org/ns/z3998/authoring/features/select/"
	exclude-result-prefixes="xs x rng d dcterms sel">

	<!--
		XSLT to build Resource Directories for Z3998 Profiles and Features
	...
	<xsl:variable name="base">http://www.daisy.org/z3998/2012/auth/<xsl:value-of select="$type"
	...
	<xsl:variable name="sch-link">./resources/z3998-<xsl:value-of select="$machinename"
		/>.sch</xsl:variable>

	<!-- link to the nvdl script -->
	<xsl:variable name="nvdl-link">./resources/z3998-<xsl:value-of select="$machinename"
	/>.nvdl</xsl:variable>
	
	<xsl:template match="x:html">

		<html prefix="rd: http://www.daisy.org/z3998/2012/vocab/resourcedirectory/# dcterms: http://purl.org/dc/terms/"
			lang="en-US"
			xml:lang="en-US">
			<head>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<p id="maintenance-agency"> This <xsl:value-of select="$type"/> is maintained by the <a
						href="http://www.daisy.org/z3998/advisory/" rel="rd:maintenance-agency"
						>ANSI/NISO Z39.98 advisory committee</a> under the auspices of <a
	...
	<xsl:template name="doIdentityURI">
		<div class="rd-section">
			...
			<p>This <xsl:value-of select="$type"/> must be identified as 
				<code property="rd:z3998-instance-name"><xsl:value-of select="$machinename"/></code> in Z39.98-AI document <xsl:value-of select="$type"/> declarations.</p>
	...
	<xsl:template name="doVersionInfo">
		<div class="rd-section">
			...
			<xsl:variable name="recURI">http://www.daisy.org/z3998/2012/auth/<xsl:value-of select="$type"/>s/<xsl:value-of select="$machinename"/>/current/</xsl:variable>
	...
	<xsl:template name="doSpecRef">
		<div class="rd-section">
			...
			<p id="spec-ref"> This version of the <xsl:value-of select="$type"/> 
				is compliant with the <a href="../../../../Z3998-2012.html" rel="rd:specification-compliance">Z39.98-2012
					Specification</a>. </p>
		</div>
	</xsl:template>
	...
	<xsl:template name="doTutorials">
		<div class="rd-section">
			...
			<p>Refer to the <a href="http://www.daisy.org/zw/ZedAI_UserPortal">Z39.98-AI
	...
	<xsl:template name="doSampleDocuments">
		<div class="rd-section">
			...
			<xsl:variable name="samplesURI"
					>http://code.google.com/p/zednext/source/browse/trunk/test/z3998/<xsl:value-of
			...
			<p>Sample documents conforming to this <xsl:value-of select="$type"/> are available from the
				Z39.98-AI Working Group's <a href="{$samplesURI}">source code repository</a>.</p>
		</div>
	</xsl:template>
		...
		<div class="rd-section">
			...
			<dl>
				<xsl:if test="x:div[@id='standalone']">
					<dt>
						z3998-feature-<xsl:value-of select="$machinename"/>-standalone.rng
					</dt>
					<dd><p>A standalone RNG schema is available for download in 
						<a href="./resources/z3998-feature-{$machinename}-standalone.zip" 
						rel="rd:informative-schema-alt">zip</a> or
						<a href="./resources/z3998-feature-{$machinename}-standalone.tar.gz" 
	...
	<xsl:template name="doCSS">
		<div class="rd-section">
			...
			<p> A <a href="./resources/z3998-base.css" rel="rd:default-css">default CSS
				...
				canonical URI <xsl:choose>
					<xsl:when test="contains($type,'profile')">
						<a href="../../../../schema/{$normative-schema-filename}">
							http://www.daisy.org/z3998/2012/schema/<xsl:value-of
								select="$normative-schema-filename" />
						</a>
					</xsl:when>
					<xsl:otherwise>
						<a href="../../../../schema/mod/{$normative-schema-filename}">
							http://www.daisy.org/z3998/2012/schema/mod/<xsl:value-of
								select="$normative-schema-filename" />
						</a>
					</xsl:otherwise>
				</xsl:choose>. </p>
		...
		<xsl:if test="boolean($default-vocab-uri) and contains($type,'profile')">
			<div class="rd-section" id="rdfa-initial-context">
				...
				<xsl:variable name="vocab-profile-uri">
					<xsl:text>http://www.daisy.org/z3998/2012/vocab/context/</xsl:text>
					<xsl:choose>
				...
				<p> The URI of the initial context document is:<br/><br/>
					<code rel="rd:z3998-rdfa-context">
	...
	<xsl:template name="doModuleList">
		<div id="module-list">
			...
			<!-- <p> Note: the canonical base URI of these modules is <a href="../../../../schema/mod/"
					>http://www.daisy.org/z3998/2012/schema/mod/</a> , where the modules are
	...
	<xsl:template name="doFooter">
		<div class="footer">
			<p id="docinfo"> This document is a <em>Z39.98-2012 Resource Directory</em>, compliant
				with <a href="http://www.w3.org/TR/rdfa-syntax/">XHTML+RDFa</a> using the <a
					href="../../../../vocab/resourcedirectory/">Z39.98-2012 Resource Directory
					Vocabulary</a>. </p>
