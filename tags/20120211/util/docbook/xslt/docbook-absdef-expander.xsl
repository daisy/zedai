...
<xsl:stylesheet version="2.0" 
   ...
   <xsl:param name="z3998-ai-canonical-uri" required="yes"/>
      ...
      <db:table role="module-overview" xml:id="{$module-doc/rng:grammar/@xml:id}-overview">
         ...
         <db:tr>
            ...
            <db:td>
               <xsl:choose>
                  <xsl:when
                     test="$module-rdf-description/dcterms:isRequiredBy[@rdf:resource=$z3998-ai-canonical-uri]"
                     >Required</xsl:when>
                  <xsl:otherwise>Optional</xsl:otherwise>
               </xsl:choose>
            </db:td>
         ...
         <db:tr>
            ...
            <db:td>
               <xsl:variable name="requires"
                  select="$module-rdf-description/dcterms:isRequiredBy[not(@rdf:resource=$z3998-ai-canonical-uri)]"
                  as="element()*"/>
               <xsl:call-template name="render-dependencies">
                  <xsl:with-param name="dependencies" select="$requires"/>
                  <xsl:with-param name="module-full-path" select="$module-full-path"/>
