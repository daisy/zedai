		...
		<ul>
			<xsl:for-each select="$xlist">
				...
				<li><a id="{$id}" href="{$href}">		        
					...
					<!--  <xsl:value-of select="substring-after(document($subdoc)//head/meta[@property='z3998-feature-nicename']/@content,$nicename-prefix)" />-->
					<xsl:value-of select="document($subdoc)//head/meta[@property='z3998-feature-nicename']/@content" />
				    </a>
				</li>
			</xsl:for-each>
		</ul>
