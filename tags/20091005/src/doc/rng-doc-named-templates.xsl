...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="presentElementShortSample">
		...
		<xsl:if test="$sample">
			<h2>Short Sample</h2>
			<!--<xsl:value-of select="hks:escapeXML()" />-->
			<!--<xsl:copy-of select="$sample"/>-->
			<pre>
				<!--<xsl:apply-templates select="doc($filename)//*[@id eq $fragment]/*"
				mode="shortsample" />-->
				<xsl:apply-templates select="doc($filename)//*[@id eq $fragment]" mode="shortsample">
					<!-- Offer the id of the container as a parameter -->
					<xsl:with-param name="container-id" as="xs:string" select="$fragment" />
				</xsl:apply-templates>
			</pre>
		</xsl:if>
		<!--<br />Short Sample: <xsl:value-of select="$filename"/>: <xsl:value-of select="doc-available($filename)"/> <br />
		Short Sample: <xsl:value-of select="resolve-uri($filename)"/>: <xsl:value-of select="doc-available(resolve-uri($filename))"/><br /> -->
	</xsl:template>
