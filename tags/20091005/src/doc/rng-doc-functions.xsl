...
<!--	<xsl:function name="hks:escapeXML" as="xs:string">
	...
	<xsl:function name="hks:elementIsAFeatureNew" as="xs:boolean">
		...
		<xsl:choose>
			...
			<xsl:otherwise>
				...
				<xsl:value-of
					select="if ($namespaceElement/@feature eq 'yes') then true() else false()" />
	...
	<xsl:function name="hks:featureNicenameFromElement" as="xs:string">
		...
		<xsl:choose>
			<xsl:when test="hks:getDocResourceForElement($element)">
				<xsl:value-of select="hks:getDocResourceForElement($element)/@featureNicename" />
			</xsl:when>
			<xsl:otherwise>
				<!-- There is no doc resource for this element, so we must look at the namespace element to get the nice name -->
				<!--				<xsl:variable name="namespaceElement" as="element()"
					select="$docResource.content//dr:namespace[@nsURI eq $element.namespace]/@featureNicename"/>-->
				<xsl:value-of
					select="$docResource.content//dr:namespace[@nsURI eq $element.namespace]/@featureNicename"
				 />
			</xsl:otherwise>
	...
	<xsl:function name="hks:getDRForElement" as="element()?">
		<!-- get the element element <element name="...">...</element>  from the simplified rng as a parameter,
			and return the proper Documentation Resource element for that element
		-->
		<xsl:param name="element" as="element()" />
		<xsl:variable name="element.ns" as="xs:string"
			select="if ($element/@ns eq '') then $defaultNamespace else $element/@ns" />
		<xsl:variable name="element.name" as="xs:string" select="$element/@name" />
		<xsl:variable name="element.parentQnames" as="xs:string*"
			select="hks:QNamesForPossibleElementParents($element)" />
		<xsl:variable name="dr.relevant" as="element()*"
			select="$docResource.content//dr:element[@name eq $element.name and @nsURI eq $element.ns]" />
		<xsl:choose>
			<xsl:when test="count($dr.relevant) = 0">
				<!-- Don't return anything -->
			</xsl:when>
				...
				<xsl:variable name="dr.withMatchingContext" as="element()*">
					<xsl:for-each select="$dr.relevant[@context]">
						<xsl:variable name="context.qnamed" as="xs:string+"
							...
							if (contains($c,':')) then $c else concat(hks:namespace2Prefix($defaultNamespace),':',$c)" />
						<xsl:if
							test="every $c in $context.qnamed satisfies some $p in $element.parentQnames satisfies $p eq $c">
							<xsl:sequence select="." />
						</xsl:if>
					</xsl:for-each>
				</xsl:variable>
		...
		<!--		<element name="pageref" docURI="../schema/mod/z3986-toc.rng#z3986.tocpageref" include="yes"/>-->
	</xsl:function>
	...
	<xsl:function name="hks:featureNicenameFromNamespace" as="xs:string">
		<xsl:param name="ns" as="xs:string" />
		<xsl:value-of select="doc($namespaceListURI)//nsl:namespace[. = $ns]/@featureNicename" />
	</xsl:function>
