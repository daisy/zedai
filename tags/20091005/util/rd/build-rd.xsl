...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="title">Resource Directory for the <xsl:value-of
		...
		<xsl:value-of select="$nicename" />, version <xsl:value-of select="$version"
		 /></xsl:variable>
	...
	<xsl:template match="x:html">
		...
		<html version="xhtml+rdfa" >			<!-- xml:base="{$base}" makes saxon uriresolver crash-->
			...
			<body>
				...
				<p class="alert">This is an early access version, made available for early testing
					and evaluation.</p>
				...
				<p id="maintenance-agency"> This <xsl:value-of select="$type" /> is maintained by
					the <a href="http://www.daisy.org/z3986/advisory/" rel="rd:maintenance-agency"
				...
				<div id="normative-references">
					...
					<div class="rd-section">
						...
						<p id="identity-uri"> The canonical identity URI of this version of this
								<xsl:value-of select="$type" /> is <code rel="rd:token-identifier"
									><xsl:value-of select="$base-identity-uri" /><xsl:value-of
									select="$version" />/</code>. </p>
					</div>

					<div class="rd-section">
						<h3>Specification Compliance</h3>
						<p id="spec-ref"> This <xsl:value-of select="$type" /> is compliant with the
								<a href="../../../../z3986-2010a.html"
					...
					<div class="rd-section">
						...
						<p> The normative schema of this <xsl:value-of select="$type" /> is <xsl:choose>
								...
								<xsl:otherwise>
									...
									<a href="./mod/{$normative-schema-filename}"
										rel="rd:normative-schema">
										<xsl:value-of select="$normative-schema-filename" />
									</a>. </xsl:otherwise>
							</xsl:choose>
						</p>
						...
						<xsl:if test="contains($type,'feature')">
							<p>Note - this feature schema does not represent an entire document
								model; it is intended for inclusion in host profiles. </p>
						</xsl:if>
						...
						<p id="version-listing">
							<a href="../index.html"> A listing of all public versions of this
									<xsl:value-of select="$type" /> is available. </a>
						</p>
					...
					<xsl:if test="boolean($default-vocab-uri) and contains($type,'profile')">
						...
						<div class="rd-section">
							...
							<xsl:variable name="vocabfilename">
								<xsl:call-template name="filename">
									<xsl:with-param name="path"
										select="translate($default-vocab-uri,'\','/')" />
								</xsl:call-template>
							</xsl:variable>

							<xsl:variable name="vocab-canon-url">
								<xsl:choose>
									<xsl:when test="contains($vocabfilename, 'base')"
										>print</xsl:when>
									<xsl:when test="contains($vocabfilename, 'book')"
										>book</xsl:when>
							...
							<p> The default RDF vocabulary for this profile is the <a
									href="../../../../vocab/{$vocab-canon-url}/"
									rel="rd:default-vocab">
									<xsl:value-of
				...
				<div id="informative-references">
					...
					<xsl:if test="contains($type, 'profile')">
						<div class="rd-section">
							...
							<p>
								<a href="./schemadoc/index.html" rel="rd:schema-documentation"
									>Schema documentation is available online</a>, and is also
								included in the <a href="#archives">downloadable archives</a>. </p>
						</div>
						...
						<div class="rd-section">
							...
							<dl>
								<xsl:for-each select="$schemas//generated">
									<xsl:if test="@type!='core-rng-reduced'">
										...
										<dd>
											<xsl:choose>
												<xsl:when test="@type='core-rng-simplified-full'"> A
												single file version of the normative RelaxNG
												schema. </xsl:when>
												<xsl:when test="@type='core-xsd-converted'"> A W3C
												XML Schema (XSD) version of the normative schema.
												This schema contains approximations. It is not
												guaranteed that instance documents valid to this
												schema will also be valid to the normative schema.
												This schema should be used for authoring purposes
												only; final validation should always be performed
												against the normative schema. </xsl:when>
							...
							<h4 id="informative-schemata-feature-reduced">Feature-reduced
								schemata</h4>
							<p>These schemas are variants of the normative RelaxNG schema that have
								one or several optional features removed.</p>
							<p>They are provided for convenience during the authoring stage.</p>
							<p>An instance document valid to a feature-reduced schema will also be
								valid to the normative schema.</p>
							<dl>
						...
						<div class="rd-section">
							...
							<p> A <a href="./resources/z3986a-basic.css" rel="rd:default-css"
									>default CSS stylesheet</a> is available for use with CSS-aware
								XML Editing applications. </p>

							<p> Refer to <a href="http://www.w3.org/TR/xml-stylesheet/"
									>http://www.w3.org/TR/xml-stylesheet/</a> for instructions on
								how to associate a CSS stylesheet with a document instance. </p>
						</div>
						...
						<div class="rd-section">
							...
							<ul>
								<li> All schemas, documentation and resources of this directory <a
										href="{$part-prefix}-{$machinename}-{$version}.zip"
										rel="rd:archive"> in <code>zip</code> format </a>. </li>
								<li> All schemas, documentation and resources of this directory <a
										href="{$part-prefix}-{$machinename}-{$version}.tar.gz"
										rel="rd:archive"> in <code>tar gzip</code> format </a>.
								</li>
							</ul>
						</div>
				...
				<div id="module-list">
					...
					<p> Note: the canonical base URI of these modules is <a
							href="../../../../schema/mod/"
				...
				<div class="footer">
					<p id="docinfo"> This document is a <em>Z39.86-2010 Resource Directory</em>,
						compliant with <a href="http://www.w3.org/TR/rdfa-syntax/">XHTML+RDFa</a>
						using the <a href="../../../../vocab/resourcedirectory/">Z3986-2010 Resource
							Directory Vocabulary</a>. </p>
				</div>
